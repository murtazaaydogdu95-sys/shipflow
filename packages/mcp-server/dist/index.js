...
// Tool: List stories
server.tool("list_stories", "List stories in the project, optionally filtered by status or sprint", {
    ...
    try {
        ...
        const stories = await apiFetch(`/api/projects/${PROJECT_ID}/stories?${params}`);
        ...
        return {
            content: [{ type: "text", text: `Error listing stories: ${error instanceof Error ? error.message : String(error)}` }],
            isError: true,
        };
    }
});
// Tool: Get story details
server.tool("get_story", "Get full details of a specific story including acceptance criteria", {
    storyId: z.string().describe("The story ID"),
}, async ({ storyId }) => {
    try {
        const story = await apiFetch(`/api/projects/${PROJECT_ID}/stories/${storyId}`);
        return {
        ...
        return {
            content: [{ type: "text", text: `Error fetching story: ${error instanceof Error ? error.message : String(error)}` }],
            isError: true,
        };
    }
});
// Tool: Get next story to work on
server.tool("get_next_story", "Get the highest-priority TODO story from the current sprint", {}, async () => {
    try {
        const stories = await apiFetch(`/api/projects/${PROJECT_ID}/stories?status=TODO`);
        if (stories.length === 0) {
            return {
                content: [{ type: "text", text: "No TODO stories found. All caught up!" }],
            };
        }
        // Sort by priority (CRITICAL first), then by creation date (oldest first)
        const priorityOrder = { CRITICAL: 0, HIGH: 1, MEDIUM: 2, LOW: 3 };
        stories.sort((a, b) => {
            const pDiff = (priorityOrder[a.priority] ?? 4) -
        ...
        return {
            content: [{ type: "text", text: JSON.stringify(stories[0], null, 2) }],
        };
    }
    catch (error) {
        return {
            content: [{ type: "text", text: `Error fetching next story: ${error instanceof Error ? error.message : String(error)}` }],
            isError: true,
        };
    }
});
// Tool: Update story status
server.tool("update_story_status", "Update the status of a story (e.g., move to IN_PROGRESS or DONE)", {
    storyId: z.string().describe("The story ID"),
    ...
    try {
        ...
        return {
            content: [{ type: "text", text: `Story ${storyId} moved to ${status}` }],
        };
    }
    catch (error) {
        return {
            content: [{ type: "text", text: `Error updating story status: ${error instanceof Error ? error.message : String(error)}` }],
            isError: true,
        };
    }
});
// Tool: Complete a story — moves to REVIEW for user approval
server.tool("complete_story", "Mark a story as complete and move it to REVIEW for user approval. Include a commit hash and summary of what was implemented.", {
    storyId: z.string().describe("The story ID"),
...
server.tool("add_note", "Add a progress note to a story", {
    storyId: z.string().describe("The story ID"),
    ...
    try {
        ...
        return {
            content: [{ type: "text", text: `Note added to story ${storyId}` }],
        };
    }
    catch (error) {
        return {
